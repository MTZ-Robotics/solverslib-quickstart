...
@Disabled
@TeleOp
public class Tele_Shooting_W_Intake_AutoFlywheelSpeed_SLmotors extends CommandOpMode {
    ...
    public final double[] TOP_FLYWHEEL_RPM_CAL = {1650,1650,1650,1900,1900};
    public final double[] BOTTOM_FLYWHEEL_RPM_CAL = {1975,1975,2100,2100,2100};
    ...
    double topFlywheelDesired = 1900;
    double bottomFlywheelDesired = 2100;
...
// Assign Variables & Objects for Control Pads
    ...
    public void initialize() {
        ...
        //MotorEx bottomFlywheel = new MotorEx(hardwareMap, "m5", Motor.GoBILDA.BARE);
        //MotorEx topFlywheel = new MotorEx(hardwareMap, "m6", Motor.GoBILDA.BARE);
        bottomFlywheel = new MotorEx(hardwareMap, "m5", Motor.GoBILDA.BARE);
        topFlywheel = new MotorEx(hardwareMap, "m6", Motor.GoBILDA.BARE);

        //bottomFlywheel = hardwareMap.MotorEx.get("m5");
        ...
        bottomFlywheel.setZeroPowerBehavior(MotorEx.ZeroPowerBehavior.FLOAT);
        topFlywheel.setZeroPowerBehavior(MotorEx.ZeroPowerBehavior.FLOAT);
    ...
    public void run() {
        ...
        */
        ...
        topFlywheelLUT .add(DISTANCE_CAL[0], TOP_FLYWHEEL_RPM_CAL[0]);
        topFlywheelLUT.add(DISTANCE_CAL[1], TOP_FLYWHEEL_RPM_CAL[1]);
        topFlywheelLUT.add(DISTANCE_CAL[2], TOP_FLYWHEEL_RPM_CAL[2]);
        topFlywheelLUT.add(DISTANCE_CAL[3], TOP_FLYWHEEL_RPM_CAL[3]);
        topFlywheelLUT.add(DISTANCE_CAL[4], TOP_FLYWHEEL_RPM_CAL[4]);
        topFlywheelLUT.createLUT();

        bottomFlywheelLUT .add(DISTANCE_CAL[0], BOTTOM_FLYWHEEL_RPM_CAL[0]);
        bottomFlywheelLUT.add(DISTANCE_CAL[1], BOTTOM_FLYWHEEL_RPM_CAL[1]);
        bottomFlywheelLUT.add(DISTANCE_CAL[2], BOTTOM_FLYWHEEL_RPM_CAL[2]);
        bottomFlywheelLUT.add(DISTANCE_CAL[3], BOTTOM_FLYWHEEL_RPM_CAL[3]);
        bottomFlywheelLUT.add(DISTANCE_CAL[4], BOTTOM_FLYWHEEL_RPM_CAL[4]);
        ...
        while (opModeIsActive())
        {
             ...
             * Chassis Control
             */
            aimError=Math.toRadians(alliance==red?45:135)-teleFollower.getPose().getHeading();
            ...
            if (runFlywheel) {
                /***
                 * //Do the order of these commands need to be swapped so you tell it a run mode and then a velocity?
                 * Is this set looking for an RPM?
                */
                bottomFlywheel.set(bottomFlywheelDesired);
                topFlywheel.set(topFlywheelDesired);
                bottomFlywheel.setRunMode(MotorEx.RunMode.VelocityControl);
                topFlywheel.setRunMode(MotorEx.RunMode.VelocityControl);
            }
            else {
         ...
         */
            ...
            telemetryData.addData("Top Flywheel Power", topFlywheel.getVelocity()/topFlywheel.getMaxRPM());
            //telemetryData.addData("Top Flywheel Ratio", topFlywheelDesired);
            telemetryData.addData("Bottom Flywheel Power", bottomFlywheel.getVelocity()/bottomFlywheel.getMaxRPM());
            telemetryData.addData("Top Flywheel Speed", topFlywheel.getVelocity());
            //telemetryData.addData("Top Flywheel Ratio", topFlywheelDesired);
            telemetryData.addData("Bottom Flywheel Speed", bottomFlywheel.getVelocity());
            ...
            if (aimBumper.isDown && targetFound) {
                ...
                if(desiredTag.ftcPose.range>100){
                ...
                //Need to find distance and angle to target based on follower
